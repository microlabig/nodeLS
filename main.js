...
const FROM = process.argv[2] ? path.join(__dirname, 'src', process.argv[2]) : path.join(__dirname, 'src', './foods'); // исходный каталог
const TO = process.argv[3] ? path.join(__dirname, 'src', process.argv[3]) : path.join(__dirname, 'src', './sort_foods'); // новый каталог
...
const readDir = (scanPath) => {
    ...
    files.forEach(file => {
        ...
        if (state.isDirectory()) {
            ...
            fs.rename(fullPath, path.join(pathTo, file), (err) => errorMessage);
        }
    });
}
...
if (fs.existsSync(FROM)) {
    fs.rmdir(FROM, { recursive: true }, (err) => errorMessage);
}
